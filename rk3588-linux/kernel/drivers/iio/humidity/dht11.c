...
#define DHT11_MIN_TIMERES	80000  /* ns */ /*34000*/
#define DHT11_THRESHOLD		49000  /* ns */ 
#define DHT11_AMBIG_LOW		28000  /* ns */ /*23000*/
#define DHT11_AMBIG_HIGH	70000  /* ns */ /*30000*/
			...
			int *val, int *val2, long m)
{
	...
	if (dht11->timestamp + DHT11_DATA_VALID_TIME < ktime_get_boottime_ns()) {
		...
		if (ret)
			goto err;
		udelay(30);
		if (gpiod_get_value(dht11->gpiod) == 0)
		{
			while(!gpiod_get_value(dht11->gpiod))
			{
				udelay(80);
				break;
			}
			...
			for(i = 0; i < 8; i++)
			{
				...
				udelay(30);
				bit = 0;
				if (gpiod_get_value(dht11->gpiod))
				{
					bit = 1;
				}
				j = 0;
